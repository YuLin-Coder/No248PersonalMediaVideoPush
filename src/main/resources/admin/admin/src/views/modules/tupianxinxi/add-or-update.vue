<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="图片名称" prop="tupianmingcheng">
          <el-input v-model="ruleForm.tupianmingcheng" 
              placeholder="图片名称" clearable  :readonly="ro.tupianmingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="图片名称" prop="tupianmingcheng">
              <el-input v-model="ruleForm.tupianmingcheng" 
                placeholder="图片名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="图片类型" prop="tupianleixing">
          <el-select :disabled="ro.tupianleixing" v-model="ruleForm.tupianleixing" placeholder="请选择图片类型">
            <el-option
                v-for="(item,index) in tupianleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="图片类型" prop="tupianleixing">
	      <el-input v-model="ruleForm.tupianleixing"
                placeholder="图片类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="图片简介" prop="tupianjianjie">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="图片简介"
                  v-model="ruleForm.tupianjianjie" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tupianjianjie" label="图片简介" prop="tupianjianjie">
                    <span>{{ruleForm.tupianjianjie}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="图片详情" prop="tupianxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.tupianxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tupianxiangqing" label="图片详情" prop="tupianxiangqing">
                    <span v-html="ruleForm.tupianxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
    ...
    return {
      ...
      type: '',
      ro:{
	tupianmingcheng : false,
	tupianleixing : false,
	tupianfengmian : false,
	tupianjianjie : false,
	tupianxiangqing : false,
	yonghuming : false,
	fabushijian : false,
	sfsh : false,
	shhf : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='tupianmingcheng'){
            this.ruleForm.tupianmingcheng = obj[o];
	    this.ro.tupianmingcheng = true;
            continue;
          }
          if(o=='tupianleixing'){
            this.ruleForm.tupianleixing = obj[o];
	    this.ro.tupianleixing = true;
            continue;
          }
          if(o=='tupianfengmian'){
	    ...
	    this.ro.tupianfengmian = true;
          ...
          if(o=='tupianjianjie'){
            this.ruleForm.tupianjianjie = obj[o];
	    this.ro.tupianjianjie = true;
            continue;
          }
          if(o=='tupianxiangqing'){
            this.ruleForm.tupianxiangqing = obj[o];
	    this.ro.tupianxiangqing = true;
            continue;
          }
          if(o=='yonghuming'){
      ...
      this.$http({
		...
		if(json.yonghuming!=''&&json.yonghuming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/tupianleixing/tupianleixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.tupianleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.tupianxiangqing = this.ruleForm.tupianxiangqing.replace(reg,'../../../springboot9zo8s/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tupianxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tupianxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
